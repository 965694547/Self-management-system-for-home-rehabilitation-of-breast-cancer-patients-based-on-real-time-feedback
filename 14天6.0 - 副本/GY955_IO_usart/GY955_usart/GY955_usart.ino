...
void final_list_1()
{
  float energy=(1.0*wq+3.0*xw+3.0*qz)/60.0*4.0;
  u8g.firstPage();  
   do {
...
void final_list_2()
{
  float energy=(1.0*wq+3.0*xw+3*qz+tb*5.0+wz*6.0)/60.0*4.0;
  u8g.firstPage();  
   do {
         // graphic commands to redraw the complete screen should be placed here  
     u8g.setFont(u8g_font_6x10);
...
{
  Mp3Module Mymp3(10,12,13);
  if(abs(Original_ROLL-ROLL)<=40)//&&abs(Attitude.Gyro_Speed_Z)<=40)
   state1=1;
...
void wq_function()
{
  ...
  if(sensorValue1>=40||sensorValue2>=40)
  {
    wq=wq+1;  
    Mymp3.VolumeSet(28);
    Mymp3.Play(7,NoWait);
...
//            case 7 : Mymp3.VolumeSet(28);Mymp3.Play(10,Wait);break;
  ...
  if((((wq==16)&&(select==1))||((xw==16)&&(select==2))||((qz==16)&&(select==3))||((tb==16)&&(select==4))||((wz==16)&&(select==5)))&&(power==1))
   {
    Mymp3.VolumeSet(28);Mymp3.Play(15,Wait);
    power=0;
   }
