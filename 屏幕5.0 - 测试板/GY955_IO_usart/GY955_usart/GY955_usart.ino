...
void final_list()
{
  ...
  // call procedure from base class, http://arduino.cc/en/Serial/Print
   u8g.setPrintPos(0,18);u8g.print("Bend elbows:");
   u8g.setPrintPos(72,18);u8g.print(qz);
  u8g.setPrintPos(0,28);u8g.print("Wrist rotation:");
   u8g.setPrintPos(90,28);u8g.print(xw);
   u8g.setPrintPos(0,38);u8g.print("Lifting arm:");
   u8g.setPrintPos(72,38);u8g.print(tb);
   u8g.setPrintPos(0,48);u8g.print("Clench fist:");
   u8g.setPrintPos(72,48);u8g.print(wq);
   u8g.setPrintPos(0,58);u8g.print("Outreach:");
   u8g.setPrintPos(55,58);u8g.print(wz);
       } while( u8g.nextPage() );
}
...
void qz_function()//现在的问题是停滞状态下数据不稳定，且放下手的时候要停滞一段时间
{
  ...
  if(abs(Original_ROLL-ROLL)<=50&&abs(Attitude.Gyro_Speed_Z)<=40)
   state1=1;
   if(abs(Original_ROLL-ROLL)>=50&&(state1==1)&&abs(Attitude.Gyro_Speed_Z)>=10)
   state1=2;
   if(abs(Original_ROLL-ROLL)>=126-40&&(state1==2)&&abs(Attitude.Gyro_Speed_Z)<=40)
   {
    state1=0;
    qz++; 
...
if(select==4)
  ...
  if(key_state==1){
    key_state=0;
  switch(select){
      case 1 :Mymp3.VolumeSet(28);Mymp3.Play(3,Wait);break;
       case 2 :Mymp3.VolumeSet(28);Mymp3.Play(4,Wait);break;
        case 3 : Mymp3.VolumeSet(28);Mymp3.Play(5,Wait);break;
         case 4 :Mymp3.VolumeSet(28);Mymp3.Play(6,Wait);break;
          case 5 : Mymp3.VolumeSet(28);Mymp3.Play(8,Wait);break;
           case 6 : Mymp3.VolumeSet(28);Mymp3.Play(9,Wait);break;
    }
  }
