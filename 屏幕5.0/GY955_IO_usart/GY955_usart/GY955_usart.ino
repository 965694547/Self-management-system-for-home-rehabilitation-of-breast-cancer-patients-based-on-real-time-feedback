...
{   
  ...
  {     
    ...
    delay(5);
    Key3_State = digitalRead(Key3);
     if(select==1){  Mymp3.VolumeSet(28);Mymp3.Play(3,Wait);}
   
     if(select==2){  Mymp3.VolumeSet(28);Mymp3.Play(4,Wait);}
  
     if(select==3){  Mymp3.VolumeSet(28);Mymp3.Play(5,Wait);}
  
     if(select==4){  Mymp3.VolumeSet(28);Mymp3.Play(6,Wait);}
  } 
...
void qz_function()//现在的问题是停滞状态下数据不稳定，且放下手的时候要停滞一段时间
{
  ...
  if(abs(Original_ROLL-ROLL)<50&&abs(Attitude.Gyro_Speed_Z)<=40)
   ...
   state1=2;
   if(abs(Original_ROLL-ROLL)>=126-20&&(state1==2)&&abs(Attitude.Gyro_Speed_Z)<=40)
   {
    state1=0;
    qz++; 
...
void tb_function()
{
  ...
  if(abs(Original_ROLL-ROLL)>=(134-30)&&(state3==2)&&abs(Attitude.Gyro_Speed_Z)<=60)
   {
    state3=0;
    tb++;
    Mymp3.VolumeSet(28);
